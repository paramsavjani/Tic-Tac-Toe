...
document
  .getElementById("3 X 3")
  .addEventListener("click", () => gridforfirsttime(3));
document
  .getElementById("5 X 5")
  .addEventListener("click", () => gridforfirsttime(5));
document
  .getElementById("7 X 7")
  .addEventListener("click", () => gridforfirsttime(7));
...
function createBoard(size) {
  ...
  const boxSize = (containerSize - (totalGap / viewportWidth) * 100) / size;
  ...
  for (let i = 0; i < size * size; i++) {
    ...
    boxtext.style.fontSize = `${boxSize / 1.3}vw`;
...
volumeIcon.addEventListener("click", function () {
  if (isMuted) {
    volumeIcon.src = "/static/volume-up.png";
...
Promise.all([
  ...
  preloadAudio("/static/gameover.mp3").then(
    (buffer) => (gameoverBuffer = buffer)
  ),
...
function startDoubleGame() {
  ...
  if (player1Name && player2Name) {
    ...
    document.querySelector(
      ".score.ply1 p:nth-child(1)"
    ).innerText = `${player1Name} (X)`;
    document.querySelector(
      ".score.ply2 p:nth-child(1)"
    ).innerText = `${player2Name} (O)`;
  ...
  function updateTurnInfo(player1Name, player2Name) {
    ...
    else document.querySelector(".info").innerText = `Start from ${obj[turn]}`;
  }
  ...
  let turn = localStorage.getItem(`turn${boardSize}`)
    ? localStorage.getItem(`turn${boardSize}`)
    : null;
  if (turn == null) {
    let x = Math.random();
    turn = x < 0.5 ? "X" : "O";
  ...
  let moveCount = localStorage.getItem(`move${boardSize}`)
    ? parseInt(localStorage.getItem(`move${boardSize}`))
    : 0;
  let player1Score = localStorage.getItem(`ply1`)
    ? parseInt(localStorage.getItem(`ply1`))
    : 0;
  let player2Score = localStorage.getItem(`ply2`)
    ? parseInt(localStorage.getItem(`ply2`))
    : 0;
  let tieScore = localStorage.getItem(`tie`)
    ? parseInt(localStorage.getItem(`tie`))
    : 0;
  ...
  function loadGameState() {
    const savedState = JSON.parse(
      localStorage.getItem(`gameState${boardSize}`)
    );
    if (savedState) {
  ...
  const checkWin = (boardSize) => {
    ...
    winPatterns.forEach((pattern) => {
      ...
      for (let i = 1; i < pattern.length; i++) {
        if (
          boxTexts[pattern[i]].innerText !== boxTexts[pattern[0]].innerText ||
  ...
  Array.from(boxes).forEach((element) => {
    ...
    element.addEventListener("click", () => {
      if (boxText.innerText === "" && !isGameOver) {
        ...
        saveGameState();
        if (turn == "X" && !isMuted) play_turn_x();
        else if (!isMuted) play_turn_0();
        turn = changeTurn();
        moveCount++;
        localStorage.setItem(`move${boardSize}`, `${moveCount}`);
        checkWin(boardSize);
        if (!isGameOver) {
          if (!checkDraw()) {
            document.querySelector(".info").innerText = `Turn for ${obj[turn]}`;
...
function startComputer() {
  ...
  document.getElementById("modeselect").style.display = "none";
  document
    .getElementById("btn-container-for-computer")
    .addEventListener("click", startGame);
  document.querySelector(
    ".score.ply2 p:nth-child(1)"
  ).innerText = `Computer (O)`;
  ...
  function rename() {
    document.getElementById("oneNameAsking").style.display = "flex";
    document.getElementById("btn-container-for-computer").innerText =
      "Save Name";
    document
      .getElementById("btn-container-for-computer")
      .addEventListener("click", function () {
  ...
  let moveWithCoputer = localStorage.getItem(`moveWithCoputer${boardSize}`)
    ? parseInt(localStorage.getItem(`moveWithCoputer${boardSize}`))
    : 0;
  let isGameOver = false;
  let playerScore = localStorage.getItem(`ply`)
    ? parseInt(localStorage.getItem(`ply`))
    : 0;
  let computerScore = localStorage.getItem(`computerScore`)
    ? parseInt(localStorage.getItem(`computerScore`))
    : 0;
  let tieScore = localStorage.getItem(`tiecomputer`)
    ? parseInt(localStorage.getItem(`tiecomputer`))
    : 0;
  const imgBox = document.querySelectorAll(".imgbox");

  function loadGameState() {
    const savedState = JSON.parse(
      localStorage.getItem(`gameStatecompter${boardSize}`)
    );
    if (savedState) {
      const boxTexts = document.querySelectorAll(".boxtext");
      boxTexts.forEach((box, index) => {
  ...
  function minimax(board, depth, alpha, beta, isMaximizing, winPatterns) {
    ...
    if (isMaximizing) {
      ...
      for (let i = 0; i < board.length; i++) {
        if (board[i] === null) {
          board[i] = COMPUTER;
          let eval = minimax(board, depth + 1, alpha, beta, false, winPatterns);
          board[i] = null;
      ...
      for (let i = 0; i < board.length; i++) {
        if (board[i] === null) {
          board[i] = PLAYER;
          let eval = minimax(board, depth + 1, alpha, beta, true, winPatterns);
          board[i] = null;

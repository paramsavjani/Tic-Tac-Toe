...
async function preloadAudio(url) {
    return fetch(url).then(response => response.arrayBuffer()).then(data => audioContext.decodeAudioData(data));
}
Promise.all([preloadAudio('/static/turn_0.mp3').then(buffer => turn0Buffer = buffer), preloadAudio('/static/turn_x.mp3').then(buffer => turnXBuffer = buffer), preloadAudio('/static/gameover.mp3').then(buffer => gameoverBuffer = buffer)]).catch(e => console.error('Error loading audio files', e));
...
function startDoubleGame() {
    ...
    function updateTurnInfo(player1Name, player2Name) {
        ...
        obj["X"] = player1Name;
        obj["O"] = player2Name;
        if (moveCount != 0) document.querySelector(".info").innerText = `Turn for ${obj[turn]}`;
